...
先为更新会话状态做一遍 `correct_text` + `filter_text`，转发前对同一文本又做一遍。
...
用 SQLite 的 `julianday('now', 'localtime')` 减去 Python `datetime.now().isoformat()` 存入的 `started_at`。
两者都依赖本机时区，一旦服务器时区变更或夏令时切换，墙钟时长就会算错。

**建议**：统一改用 UTC（存 `time.time()` 的 epoch 秒最简单）。
